                            ...
                            <div class="sc-padding-medium uk-margin sc-round sc-border md-bg-grey-100 uk-grid-small"
                                ...
                                <div class="uk-width-1-5@s">
                                    <select v-model="searchParkingSite" class="uk-select">
                                        ...
                                        <option v-for="opt in siteOpts" :key="opt.id" :value="opt.id">
                                            {{ opt.text }}
                                        </option>
                                    </select>
                                </div>
        ...
        async exportData() {
            ...
            await this.$axios.$get(this.config.apiUrl + '/payLogs', searchData).then(response => {
                response.data.rows.forEach((key, index) => {
                    let ticketTitle = key.discountTicket && key.discountTicket.ticketTitle ? key.discountTicket.ticketTitle:  '자동결제'
