          ...
          <div class="payLog uk-grid">
            ...
            <div class="payLog-block uk-width-1-2">
              ...
              <div class="payLog-block__content">
                ...
                <button v-if="defaultDetail.status === 10 && defaultDetail.cancelStatus < 0"
                        @click="cancelPayment(defaultDetail.uid)">결제 취소
                </button>
              </div>
            </div>
            <div class="payLog-block uk-width-1-2">
            ...
            <div v-if="defaultDetail.cancelStatus >= 0" class="payLog-block uk-width-1-2">
              ...
              <div class="payLog-block__content">
                ...
                <button v-if="defaultDetail.cancelStatus === 0 && defaultDetail.status !== -20"
                        @click="refundProcess(defaultDetail.uid)">승인
                </button>
                <button v-if="defaultDetail.cancelStatus === 0 && defaultDetail.status !== -20"
                        @click="refundReject(defaultDetail.uid)">거절
                </button>
              </div>
            </div>
            <div v-if="defaultDetail.cancelStatus >= 0" class="payLog-block uk-width-1-2">
  ...
  methods: {
    async openDetail(props) {
      if (props) {
        ...
        switch (data.status) {
          case 0:
            data.statusName = "결제대기"
            break
          case 10:
            data.statusName = "결제완료"
            break
          case -10:
            data.statusName = "결제실패"
            break
          case -20:
            data.statusName = "결제취소"
            break
        }
        switch (data.cancelStatus) {
          case -1:
            data.cancelStatusName = "신청없음"
            break
          case 0:
            data.cancelStatusName = "환불신청"
            break
          case 10:
            data.cancelStatusName = "환불완료"
            break
          case -10:
            data.cancelStatusName = "환불거절"
            break
        }
        switch (data.clientStatus) {
          case "used":
            data.clientStatusName = "사용완료"
            break
          case "expired":
            data.clientStatusName = "기간만료"
            break
          case "refunding":
            data.clientStatusName = "환불신청"
            break
          case "refunded":
            data.clientStatusName = "환불완료"
            break
          case "paid":
            data.clientStatusName = "결제완료"
            break
        }
    ...
    refundProcess(uid) {
      UIkit.modal.confirm("승인 하시겠습니까?").then(() => {
        this.$axios.$post(this.config.apiUrl + '/refundApprove', {
    ...
    refundReject(uid) {
      UIkit.modal.confirm(`거절 하시겠습니까?`).then(() => {
