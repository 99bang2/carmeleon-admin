<template>
    ...
    >
        ...
        >
            ...
            >
                <ScMenuList v-if="item.submenu && item.submenu.length" v-show="item.isOpen"
                            :menu-data="item.submenu"></ScMenuList>
            </transition>
        </li>
    </ul>
</template>
    ...
    export default {
        ...
        created() {
            this.menuItems.forEach(k => {
                if (typeof k.grade !== 'undefined' && k.grade.indexOf(String(this.$auth.user.grade)) === -1) {
                    k.hide = true
                }
